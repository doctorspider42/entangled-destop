...
# gpu_remote_protocol, gpu_blob, venus_window, input_device, snd_control,
# snd_device, usernet_frames, snapshot_parse).
...
| `venus_window` | the Venus host-visible window (VEN-2003): the real `machine_x86::shm::ShmWindow` in renderer-mapped mode, with a recording `GpaMapper` and real host pages, driven by arbitrary BAR bases (hostile ones included) and arbitrary region-relative map/unmap offsets. Where `gpu_blob` fuzzes a guest offset indexing *inside* a mapping, this one fuzzes a guest offset **becoming** a hypervisor mapping. Invariants re-derived from outside, against what the hypervisor was told: every live range inside the current placement, no two overlapping, nothing mapped while the BAR decodes nothing, the count under `MAX_HOST_RANGES`, one region's offsets never reaching another's span, and a dropped window leaving the hypervisor holding nothing. It found a real one within two minutes — a zero-length bounds check let region 1 unmap region 2's mapping |
