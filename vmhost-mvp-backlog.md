...
| GAME-2104 | Pad: `virtio-input` z osiową mapą kontrolera + przechwytywanie z hosta (XInput/evdev), hotplug | P1 | zrobione (`Profile::Gamepad`, `[gamepad] enabled`): deskryptor skopiowany z jądrowego `xpad` (11 przycisków `BTN_GAMEPAD`, `ABS_X/Y/RX/RY/Z/RZ` + hat), więc `joydev` daje `/dev/input/js0`, udev taguje `ID_INPUT_JOYSTICK`, a SDL wyprowadza mapowanie z samych capability — bez wpisu w bazie kontrolerów i bez podszywania się pod VID/PID Microsoftu. Host: evdev pisany ręcznie (`libc`, cztery ioctl-e — bez `gilrs`) i XInput; źródła raportują *stan*, pompa liczy różnicę, więc hotplug w obie strony wychodzi sam. Bez martwej strefy po stronie hosta. Akceptacja na żywym gościu (`tests/boot/tests/gamepad.rs`, dwa boot-y): jądro rejestruje `js=1 keys=11 axes=8 absx=-32768:32767:16:128`, a wpięcie/wypięcie pada w trakcie pracy daje dokładnie trzy raporty (wciśnięcie, zwolnienie *przy wypięciu*, nowy pad) i ciszę potem. Wymagało `CONFIG_INPUT_JOYDEV=y` w jądrze bootstrap — to osobny symbol od `CONFIG_INPUT_EVDEV`. Przełącznik w GUI jest już w edytorze maszyny (*Network & display ▸ Give this machine a gamepad*), a instalatory włączają pada w profilu instalowanego systemu. Domknięte w kolejnej turze: **drugi gracz** (`[gamepad] players = 1..=4`, po jednym slocie virtio na gracza — arytmetyka slotów opisana przy `control_api::GamepadSection`; padów nie da się upchać w jedno urządzenie, bo jedno urządzenie virtio-input to jedno urządzenie evdev). Który pad hosta należy do którego gracza rozstrzyga wspólny `gamepad::PadRoster`: wypełniamy po kolei, nigdy nie odbieramy i nigdy nie awansujemy — wypięcie pada gracza 1 zostawia pad gracza 2 na miejscu. Pady różnią się wyłącznie seryjnym (`VIRTIO_INPUT_CFG_ID_SERIAL` → `idev->uniq`). **`js0` naprawione**: reguła `joydev_dev_is_absolute_mouse()` to trzy dokładne porównania bitmap, a `EV_REL` jest dopuszczalne tylko razem z `EV_MSC` — więc samo usunięcie `BTN_SIDE`/`BTN_EXTRA` (tak sugerował stary komentarz) nic by nie dało, co zmierzono na żywym gościu. Tablet ma teraz trzy przyciski i jeden bit `MSC_SCAN`, a Back/Forward myszy jadą do klawiatury jako `KEY_BACK`/`KEY_FORWARD`. **Rumble: niewykonalne, nie „nie zrobione”** — `virtio_input.c` w Linuksie nigdy nie pyta o `EV_BITS[EV_FF]` i nie woła `input_ff_create()` (sprawdzone dla v6.12, v6.16, v6.17 i master), a specyfikacja virtio-input nie ma kanału na `struct ff_effect` (kolejka status niesie 8-bajtowy `type/code/value`). Oba fakty są asertowane na prawdziwym jądrze w `tests/boot/tests/gamepad.rs`; host-owy `RumbleSink` napisany dziś byłby martwym kodem. Odblokowanie wymaga w tej kolejności: rozszerzenia specyfikacji, sterownika w gościu, dopiero potem backendu hosta.
